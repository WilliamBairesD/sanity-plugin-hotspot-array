...
const imageStyle = { width: "100%", height: "auto" }, VALID_ROOT_PATHS = ["document", "parent"], exts = ["psd"], pattern = new RegExp(`\\.(${exts.join("|")})$`, "i"), getCloudinaryAssetUrl = (asset) => {
  ...
  if (urlParts.length < 2) return assetUrl;
  const lastSlashIndex = urlParts[1].lastIndexOf("/"), existingParams = urlParts[1].substring(0, lastSlashIndex), params = [];
  return existingParams.includes("f_") || params.push("f_auto"), existingParams.includes("q_") || params.push("q_auto"), params.length > 0 ? `${urlParts[0]}/upload/${params.join(",")}/${urlParts[1]}` : assetUrl;
};
function ImageHotspotArray(props) {
  const { value, onChange, imageHotspotOptions, schemaType, renderPreview } = props, sanityClient = sanity.useClient({ apiVersion: "2022-01-01" }), imageHotspotPathRoot = react.useMemo(() => (VALID_ROOT_PATHS.includes(imageHotspotOptions.pathRoot ?? "") ? imageHotspotOptions.pathRoot : "document") === "document" ? [] : props.path.slice(0, -1), [imageHotspotOptions.pathRoot, props.path]), rootObject = sanity.useFormValue(imageHotspotPathRoot), resolveInitialValueForType = sanity.useResolveInitialValueForType(), resolveInitialValue = react.useCallback(
    ...
    if (imageHotspotOptions.origin === "cloudinary" && hotspotImage) {
      const cloudinaryHostspotImage = hotspotImage, aspectRatio = cloudinaryHostspotImage.width / cloudinaryHostspotImage.height, width = 1200, height = Math.round(width / aspectRatio);
    ...
    return null;
  }, [hotspotImage, imageHotspotOptions.origin, sanityClient]), handleHotspotImageClick = react.useCallback(
    async (event) => {
